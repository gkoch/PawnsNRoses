...
//        testFiles.add("dm004.epd");
        ...
        // maxNodeCount=500: 132663     2080169
        final long expectedNodeCount = Long.parseLong(System.getProperty("searchTest.expectedNodeCount", "-1"));
        if (expectedNodeCount >= 0) {
            assertTrue("too high total node count: " + totalNodeCount + " vs " + expectedNodeCount,
                totalNodeCount < expectedNodeCount * 1.1);
            assertTrue("too low total node count: " + totalNodeCount + " vs " + expectedNodeCount,
                totalNodeCount > expectedNodeCount * 0.9);
        }
        final double processTime = ((double) (endTime - startTime)) / 1000;
        System.out.printf("Solved %d mates, in %.2f seconds. Total node count: %d (%.1f nodes/s), test expected: %d," +
            " previous best: %d\r\n",
            task.getTestCount(), processTime, totalNodeCount, ((double) totalNodeCount) / processTime,
            task.getTotalExpectedNodeCount(), expectedNodeCount);
    }
...
//            System.out.println("Transposition table entry counts: " +
//                Arrays.toString(engine.getTranspositionTable().getEntryCounts()));
            totalNodeCount += engineNodeCount;
