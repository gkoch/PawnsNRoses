...
public class StalemateTest {
    ...
    public static void main(String[] args) throws IOException {
        ...
        if (expectedNodeCount >= 0) {
            assert totalNodeCount < expectedNodeCount * 1.1:
                "too high total node count: " + totalNodeCount + " vs " + expectedNodeCount;
            assert totalNodeCount > expectedNodeCount * 0.9:
                "too low total node count: " + totalNodeCount + " vs " + expectedNodeCount;
        }
        final double processTime = ((double) (endTime - startTime)) / 1000;
        System.out.printf(
            "Solved %d stalemates, in %.2f seconds. Total node count: %d (%.1f nodes/s),  previous best: %d\r\n",
